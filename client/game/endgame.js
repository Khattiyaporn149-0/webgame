...
// Safe for dynamic import; no external CSS/HTML dependencies
// Updated: remove ES module export for direct browser compatibility
...
(function () {
  function ensureOverlay(detail = {}) {
    ...
    const goExit = () => { try { location.href = redirectTo; } catch { } };
  ...
  // ✅ expose globally
  window.showEnd = function (detail) {
    try { ensureOverlay(detail); } catch (e) { console.error('End overlay failed', e); }
  };

  // listen for custom event
  try {
    window.addEventListener('game:end', (e) => window.showEnd(e?.detail || {}));
  } catch { }
})();
